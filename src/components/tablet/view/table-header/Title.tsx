    ...
    return (
        <>
            <div className="table-header-title-section">
                <h2 className="table-title">Example Tablet</h2>

                {dataLoading && (
                    <DataStatusIndicator type="loading" message="Cargando datos..." />
                )}

                {dataError && (
                    <DataStatusIndicator type="error" message={`Error: ${dataError}`} onRetry={refetchData} />
                )}
            </div>
